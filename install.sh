...
THEMES="$HOME/.themes"
SCR_BIN="/usr/local/bin"
for DIR in "$BIN_DIR" "$DIRSCR" "$DOWNLOADS" "$THEMES" "$SCR_BIN" "$WALLPAPERS" "$LIVE"; do
    if [ ! -d "$DIR" ]; then
...
echo "::: Removing the autogenerated warning in Hyprland config if exists..."
...
echo "::: Enabling copy/paste shortcuts in Kitty if config doesn't exist"
